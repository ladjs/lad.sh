...
async function login(ctx, next) {
  ...
  await passport.authenticate('local', async (err, user, info) => {
    ...
    if (!user) {
      if (info) throw info;
      throw new Error(ctx.translateError('UNKNOWN_ERROR'));
    }
...
async function loginOtp(ctx, next) {
  await passport.authenticate('otp', (err, user) => {
    if (err) throw err;
    if (!user) throw Boom.unauthorized(ctx.translateError('INVALID_OTP_PASSCODE'));
...
async function resetPassword(ctx) {
  ...
  if (!user) throw Boom.badRequest(ctx.translateError('INVALID_RESET_PASSWORD'));
